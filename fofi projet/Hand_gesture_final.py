...
while True:
    ...
    retval2,thresh1 = cv2.threshold(gray,70,255,cv2.THRESH_BINARY) # THRESHOLDING IMAGE
    hand = hand_cascade.detectMultiScale(thresh1, 1.1, 5) # DETECTING HAND IN THE THRESHOLDE IMAGE
    mask = np.zeros(thresh1.shape, dtype = "uint8") # CREATING MASK
    
    for (x,y,w,h) in hand: # MARKING THE DETECTED ROI
    	cv2.rectangle(img,(x-30,y-30),(x+w+30,y+h+30), (122,122,0), 2) 
    	cv2.rectangle(mask, (x-30,y-30),(x+w+30,y+h+30),255,-1)
